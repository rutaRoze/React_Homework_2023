...
function ProductList() {
  ...
  const clickShowModal = (title, description, quantity) => {
    setShowModal(true);
    setShowTitle(title);
  ...
  return (
    <>
      <div className="container text-center position-relative">
        <div className="row row-cols-3">{mappedProducts}</div>
      </div>
      ...
      {showToast && (
        <ProductToast hideToast={clickHideToast} title={showTitle} />
      )}
    </>
  );
}
